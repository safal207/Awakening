...
internal static class DistrictScene
{
    ...
    internal static readonly Box2 ParkedTramBounds = new(-1f, 15.8f, 9f, 18.2f);
    internal static Box2 TramBounds(FirstDistrictEpisode episode) =>
        new(ParkedTramBounds.Min + episode.TramOffset.Xz, ParkedTramBounds.Max + episode.TramOffset.Xz);
    internal static bool TramPresent(FirstDistrictEpisode episode, int day) => day == 1 && !episode.TramGone;
    ...
    internal static List<float> Build(FirstDistrictEpisode episode, int day)
    {
        ...
        SceneGeometry.Box(v,new(11.3f,2.9f,12.14f),new(0.4f,0.3f,0.04f),
            day > 1 || episode.Finished ? green : amber);
        ...
        for (int rail = 0; rail < 2; rail++)
            SceneGeometry.Box(v,new(-165f,0.025f,16.25f + rail*1.5f),new(205f,0.02f,0.06f),new(0.46f,0.49f,0.47f));
        return v;
    }

    internal static List<float> BuildTram()
    {
        var v = new List<float>(12000);
        Vector3 metal = new(0.19f,0.23f,0.22f);
